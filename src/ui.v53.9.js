...
import { t, getLang } from "./i18n.v53.9.js";
...
export function setSaveLocked(locked, label = "") { const saveBtn = $("saveBtn"); if (!saveBtn) return; saveBtn.disabled = !!locked; saveBtn.dataset.originalLabel ||= saveBtn.textContent; saveBtn.textContent = locked ? (label || t('save')) : saveBtn.dataset.originalLabel; $$('[data-step], [data-qty-index], #receiveDateInput', dom.itemPanel).forEach((el) => { el.disabled = !!locked; }); }
...
function itemMetaModel(mode, item, stock, order) {
  if (mode === 'count') {
    const zone = escapeHtml(translateInlineZoneText(item.mode_target_label || item.count_zone_th || item.mode_target || item.count_zone || '-'));
  ...
  if (mode === 'issue') {
    return {
      primary: `ตัดจาก ${escapeHtml(translateInlineZoneText(item.mode_target_label || item.issue_source_th || item.mode_target || item.issue_source || 'location'))}`,
      secondary: `${escapeHtml(translateInlineZoneText(item.sub_category_th || item.sub_category || ''))}`,
    };
  }
  if (mode === 'receive') {
    return {
      primary: `รับเข้า ${escapeHtml(translateInlineZoneText(item.mode_target_label || item.receive_target_th || item.mode_target || item.receive_target || '-'))}`,
      secondary: `${escapeHtml(translateInlineZoneText(item.sub_category_th || item.sub_category || ''))}`,
    };
  }
  return {
