...
function pickTargetLabel(row) {
  return normalizeLabel(
    row.mode_target_label || row.mode_target_th ||
    row.stock_location_th || row.location_th ||
    row.count_zone_th || row.receive_target_th || row.issue_source_th ||
    translateSystemLabel(row.mode_target || row.stock_location || row.location || row.count_zone || row.receive_target || row.issue_source, 'ไม่ระบุโซน'),
    'ไม่ระบุโซน'
  );
}
...
function buildDedupKey(row) {
  return [
    ...
    normalizeLabel(row.mode || ""),
    normalizeLabel(row.mode_target || row.stock_location || row.location || row.count_zone || row.receive_target || row.issue_source || ""),
    pickTargetLabel(row),
    pickSubLabel(row),
    normalizeLabel(row.item_name_th || row.item_name, "ไม่มีชื่อ")
  ].join("|");
}
